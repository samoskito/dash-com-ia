...
- Configuracao segura do token CAPI por workspace implementada: `PUT /integrations/meta/capi-token` salva ou limpa o token com a criptografia Meta existente, exige permissao de gerenciamento de integracoes, audita `meta.capi_token.updated` sem gravar segredo completo e a aba `Integracoes` mostra apenas `Token CAPI configurado/ausente` com campo vazio para substituir/remover.
...
- Continuar a proxima rodada com: parser mais robusto de Uazapi/CTWA/etiquetas, preparar adapter minimo para WhatsApp Cloud API, endurecer contrato de datas/formulas dos relatorios e validar sincronizacao/envio CAPI em contas reais quando as credenciais estiverem disponiveis.
