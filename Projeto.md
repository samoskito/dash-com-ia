...
- A criacao real da cobranca no Asaas agora esta plugada ao checkout, mas depende de `ASAAS_BASE_URL`, `ASAAS_API_KEY` e `Workspace.asaasCustomerId`. O backoffice ja possui endpoint protegido para configurar o customer Asaas por workspace; ainda falta a tela dedicada/listagem de workspaces.
...
- Backoffice de workspaces ganhou configuracao operacional de billing: `GET/PATCH /backoffice/workspaces/:workspaceId/billing` permite aos administradores da plataforma visualizar/atualizar `asaasCustomerId`, necessario para criar cobrancas reais no Asaas sem abrir o banco.
