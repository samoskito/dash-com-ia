...
- Revisao de Relatorios do Bloco 4.5 implementada em 2026-07-13. A aba Anuncios agora exibe somente nesse nivel a miniatura do criativo, com preview no hover em desktop, modal ampliado no clique e fallback quando a imagem nao existe; o nome do anuncio virou uma selecao real que preserva campanha, conjunto e anuncio na URL e filtra a hierarquia cruzada. Campanhas, conjuntos e anuncios receberam controle de ativar/pausar, e o orcamento e editavel apenas no proprietario correto: campanha em CBO e conjunto em ABO. As escritas usam o token Meta somente no backend, exigem permissao owner/admin, confirmacao explicita e estado/valor esperado para impedir sobrescrita obsoleta; sucesso atualiza o snapshot e gera auditoria, enquanto falha Graph nao altera o banco. A migration `20260713233000_meta_report_operations` persiste miniatura, status efetivo e orcamentos e foi aplicada localmente. A suite completa passou com 57 testes shared, 495 da API e 115 do web, total de 667; typechecks, `prisma validate`, build Next e compilacoes isoladas de shared/Nest passaram. A inspecao em 1440 x 900 e 390 x 844 confirmou selecao, preview, modal e tabela rolavel sem overflow da pagina; nenhuma escrita real foi executada na Meta. Ordem de producao: aplicar a migration, publicar API/web, executar `Sincronizar Meta` para preencher miniaturas e orcamentos, confirmar que o token possui `ads_management` e validar primeiro uma entidade de teste pausada antes de liberar o uso amplo. O build agregado encontrou somente o lock local conhecido `EPERM` da DLL Prisma porque a API de desenvolvimento estava ativa.
- Qualidade dos criativos em Relatorios corrigida em 2026-07-13. `MetaAd.thumbnailUrl` continua guardando a imagem leve da tabela, enquanto a migration `20260714003000_meta_ad_preview_url` adiciona `previewUrl` para hover e modal. Durante `Sincronizar Meta`, a API busca cada `AdCreative` em lotes de ate 50 com `thumbnail_width/thumbnail_height=1200`, prioriza `image_url` e imagens dos specs para criativos estaticos e, quando encontra `video_id`, consulta `/VIDEO_ID/thumbnails` e escolhe a maior area disponivel. Falhas desse enriquecimento nao interrompem a sincronizacao e preservam a miniatura atual como fallback. Hover e modal foram ampliados; o palco contem imagens verticais ou horizontais inteiras e tambem escala o fallback pequeno, sem corte ou overflow. A migration foi aplicada localmente e o schema esta atualizado; passaram 57 testes shared, 496 da API e 115 do web, total de 668, alem dos builds Nest/Next e `git diff --check`. A inspecao automatizada em 1440 x 900 e 390 x 844 confirmou preview de 1200 x 1600, modal responsivo e ausencia de overflow. Ordem de producao: aplicar a nova migration, publicar API/web e executar uma vez `Sincronizar Meta` para preencher `previewUrl` nos anuncios ja existentes.
- Eventos finais do conector externo sem `ctwa_clid` foram separados de bloqueios corrigiveis em 2026-07-13. A ingestao agora grava `status=not_eligible`, sem erro e sem fila CAPI; a migration `20260714010000_external_capi_not_eligible` reclassifica os registros externos existentes que estavam em `pending_meta_context`. Esses eventos continuam vinculados aos leads e contam nos relatorios de negocio, mas aparecem como `Nao elegiveis` na Auditoria Meta, nao aceitam retry manual, nao entram em `Bloqueados` e nao impedem o gate. Ao mesmo tempo, nao contam como amostra operacional valida para autorizar o corte, portanto um tipo de evento que possua apenas ocorrencias sem identificador de clique permanece em `Coletando`. Eventos internos continuam usando `pending_meta_context` quando ainda podem receber enriquecimento. Verificacao local: 57 testes shared, 499 da API e 115 do web passaram, total de 671; `pnpm typecheck`, build Nest isolado e `git diff --check` passaram. O build agregado encontrou apenas o lock local conhecido `EPERM` da DLL Prisma porque a API de desenvolvimento estava ativa. Ordem de producao: aplicar a migration e publicar API/web; nao e necessario alterar o n8n nem tentar reenviar os eventos sem identificador de clique.
- Corte CAPI por evento implementado em 2026-07-13. A migration `20260714020000_external_capi_event_cutover` registra a posse de envio separadamente para `conversation_started`, `qualified_lead` e `purchase`, com horario/operador de ativacao e rollback. Os eventos que ja estavam em `Aguardando` antes da ativacao eram evidencias do modo sombra, nao uma fila para disparo: ao assumir um tipo, os `ready_to_send` anteriores ao corte passam para `shadow_observed` e aparecem como `Em sombra`, sem chamada Meta. Apenas eventos novos, ocorridos a partir de `activatedAt`, podem entrar na fila. O worker tambem verifica essa posse imediatamente antes da chamada Graph, impedindo retry manual ou job antigo de furar o corte.
- A ativacao e o rollback sao operacoes transacionais exclusivas do backoffice. `Assumir envio` exige a confirmacao `ASSUMIR ENVIO`, valida Meta/destino/gate, arquiva o passivo sombra e cria o corte; `Reverter CAPI` exige `REVERTER CAPI`, arquiva pendencias nao enviadas e devolve a responsabilidade ao n8n. O contrato de `event_id` foi alinhado ao legado (`lead_<wamid>`, `qualified_<ctwa_clid>` e `purchase_<sha256 telefone>_<data local>`) para a Meta deduplicar um eventual intervalo curto de sobreposicao. Flags globais nao podem mais habilitar CAPI diretamente por PATCH, e conector/sync nao pode ser desligado enquanto houver corte ativo.
- Procedimento operacional aprovado no codigo: publicar migration/API/web; sincronizar; assumir primeiro apenas um tipo pronto; assim que a plataforma confirmar, desligar somente o node HTTP que envia esse evento a Meta no n8n, preservando webhook, insert em `wpptrack_tracking_events` e demais efeitos; gerar um evento real novo e validar WppTrack + Events Manager antes de repetir com o proximo tipo. Em rollback, reativar primeiro o node Meta no n8n e depois usar `Reverter CAPI`. Verificacao local desta implementacao: schema Prisma valido, 57 testes shared, 507 da API e 115 do web passaram, total de 679; build Nest e build Next isolados passaram. O build agregado encontrou somente o lock local conhecido `EPERM` da DLL Prisma por processo em uso.
- Primeiro corte real concluido em producao em 2026-07-13 para `conversation_started`: o backoffice mostra `CAPI parcial`, o WppTrack assumiu apenas Conversas e os eventos anteriores foram arquivados corretamente como `Em sombra`. No n8n, o node HTTP `Envia conversao de Lead1` foi removido do caminho, enquanto `Filtra page_id e pixel_id1` segue direto para `atualizar dados1` e `atualizacao lead no banco1`, preservando os efeitos de negocio. O proximo checkpoint e observar um lead CTWA real novo tanto na Auditoria Meta quanto no Events Manager antes de assumir `qualified_lead` ou `purchase`.
- Polimento operacional de Leads, Relatorios e Integracoes implementado em 2026-07-13. A etapa do lead agora e progressiva (`LeadSubmitted` < `QualifiedLead` < `Purchase`), portanto uma conversa posterior nao rebaixa visualmente um lead ja qualificado ou convertido. A tabela de Leads substituiu as colunas concorrentes Evento/Status por `Etapa atual` com rotulos humanos; filtros operacionais e de funil ficaram nomeados e organizados em duas faixas estaveis. O seletor de sidebar recolhida foi limitado a chips dentro da propria sidebar, impedindo que os totais do cabecalho virem apenas bolinhas. Em Relatorios, `Incluir`, `Excluir` e `Resetar` agora mostram a classificacao atual, estado selecionado e toast de sucesso/erro; em Integracoes, a acao da conta Meta ganhou coluna e alinhamento dedicados. Verificacao: 508 testes da API e 117 do web passaram, alem dos typechecks de ambos; o pretypecheck Prisma encontrou somente o lock Windows conhecido `EPERM` da DLL em uso, e a compilacao TypeScript direta da API passou.
- Correcao final da revisao WhatsApp em Relatorios implementada em 2026-07-13. Todas as linhas agora usam as etapas dinamicas do cabecalho como esquema canonico, preenchendo com zero as metricas ausentes para impedir que `Incluir`, `Excluir` e `Resetar` avancem sobre as colunas de ROAS. `Revisao WhatsApp` possui uma ultima coluna estrutural propria em campanhas, conjuntos, anuncios, estados vazios e rodape. A API devolve a classificacao efetivamente persistida e o controle cliente atualiza imediatamente o selo e o botao selecionado, inclusive `Excluido manualmente`; o item continua auditavel pelo filtro `Excluidas`. Nao ha migration. Verificacao: 508 testes da API e 119 do web passaram, alem dos typechecks, build Next de producao, compilacao Nest isolada e `git diff --check`.
- Compatibilidade de deploy da revisao WhatsApp reforcada em 2026-07-13. Durante uma publicacao desacoplada, a API anterior ainda respondia apenas `{ ok: true }` mesmo salvando a revisao, enquanto o frontend novo aguardava `whatsappClassification`; o resultado era toast de sucesso sem alterar o selo ou habilitar `Resetar`. A acao web agora usa o override manual confirmado como fallback, preserva a classificacao efetiva quando a API nova a informa e mostra estado automatico generico ao resetar contra uma API antiga. O teste de controller passou a proteger o contrato completo e o web cobre explicitamente respostas antigas e novas. Verificacao: 508 testes da API, 122 do web, typechecks de ambos e build Next de producao passaram.
- Auditoria detalhada dos Eventos Meta implementada em 2026-07-13. Cada evento novo passa a guardar a entrada normalizada recebida do conector, o corpo exato enviado pela CAPI e a resposta exata da Meta; telefones ficam mascarados no snapshot de entrada e chaves sensiveis sao removidas novamente na leitura. A rota `GET /reports/conversions/audit/:eventId` exige o workspace atual e devolve motivo tecnico, campos ausentes e snapshots com procedencia explicita (`stored`, `historical_summary`, `reconstructed` ou `unavailable`). Na tela Eventos Meta, cada linha ganhou `Inspecionar`, com modal de Resumo, Entrada, Envio Meta e Resposta Meta, visualizador JSON e copia. Eventos `Nao elegivel` mostram por que nao foram enviados e como chegaram; registros historicos continuam auditaveis sem serem apresentados como payload original. Publicacao exige aplicar a migration `20260714030000_conversion_event_audit_payloads`, que adiciona `sourcePayload` e `providerRequestPayload` ao `ConversionEventLog`.
- Contrato Barbieri/Kinbox alterado para trafego pago estrito em 2026-07-13. O workflow oficial Meta agora executa `Separar mensagens -> Somente mensagens com CTWA` antes de consultar historico, gravar `wpptrack_tracking_events`, criar lead ou seguir os efeitos legados; mensagens organicas continuam apenas no inbox bruto de recuperacao e nao entram nas tabelas de negocio. O artefato preserva o corte ja ativo: `Filtra page_id e pixel_id` segue direto para `atualizar dados`, enquanto o antigo HTTP `Envia conversao de Lead` fica desconectado para nao duplicar CAPI com o WppTrack. A API repete a politica para o conector `kinbox_mysql`: `conversation_started` sem CTWA retorna `filtered` antes de qualquer leitura/escrita e a projecao de leads ignora linhas sem CTWA sem travar o cursor. A migration PostgreSQL `20260714040000_external_paid_traffic_only` remove somente leads/eventos externos organicos sem CTWA, nao enviados e sem qualquer evidencia paga. No MySQL, `docs/setup/external-mysql/migrations/20260713_paid_traffic_only.sql` limpa as linhas antigas sem CTWA das tabelas de negocio e recria as duas views pagas, preservando `wpptrack_webhook_inbox`. Depois do deploy, substituir o workflow ativo pelo JSON corrigido, executar o SQL externo com sufixo `barbieri`, publicar a migration/API e sincronizar uma vez para confirmar que os 12 organicos desapareceram e nao retornam. Verificacao local: validador do workflow aprovado, 58 testes shared, 513 da API e 122 do web passaram (693 no total), alem de Prisma validate, compilacao Nest, typechecks e build Next de producao.
...
9. Validar o primeiro `conversation_started` novo apos o corte de 2026-07-13 na Auditoria Meta e no Events Manager. Somente depois dessa confirmacao preparar uma amostra real aceita de `qualified_lead` e `purchase`, assumindo um tipo por vez e mantendo eventos antigos `Em sombra` ou `not_eligible` fora de reenvio.
