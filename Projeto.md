...
- Estado atual da ordem aprovada: **Bloco 0 implementado e em observacao de producao; Bloco 0.5 deployado e configurado para Barbieri, aguardando apenas validar/reconciliar o primeiro sync sombra; Bloco 1 - Visao Geral sera retomado depois da paridade com dados reais**.
...
- Proximo gate operacional do Bloco 0.5: deploy da migration/API, adicionar ENVs do conector, aplicar o SQL padronizado no MySQL do cliente, criar usuario read-only, cadastrar/testar o conector e reconciliar o sync sombra. O envio CAPI antigo do n8n so pode ser desativado depois dessa aprovacao.
...
- Operacao Barbieri avancou em 2026-07-11: workspace proprio provisionado, conector MySQL testado/ativado em modo sombra e sync manual solicitado. O backoffice agora mostra status terminal e totais agregados de importados, duplicados, rejeitados e pendentes; apos o clique ele acompanha somente a saude do conector ate concluir, sem recarregar a pagina inteira. Formularios administrativos passaram a usar server actions com retorno local e aviso visual, preservando exatamente a posicao de rolagem.
...
9. Reconciliar o primeiro sync sombra Barbieri e, depois, executar o dual-write/cutover dos eventos n8n sem desligar o envio Meta antigo antes da aprovacao dos totais.
