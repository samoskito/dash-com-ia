...
- Aplicacao operacional pendente: publicar o backend, substituir no n8n os workflows ativos de Lead Qualificado e Purchase pelos dois JSONs corrigidos e executar `Sincronizar` no conector MySQL Barbieri uma vez. Essa sincronizacao limpa/reassocia os sete registros atuais sem exigir SQL manual. Verificacao local: 453 testes da API, 99 do web e 54 compartilhados passaram (606 no total), alem de `pnpm lint` e parse dos dois exports n8n.
